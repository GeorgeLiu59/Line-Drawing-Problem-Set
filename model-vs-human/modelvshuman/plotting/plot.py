...
def plot_shape_bias_matrixplot(datasets,
    ...
    for dmaker in decision_maker_fun(df):
        df_selection = df.loc[(df["subj"].isin(dmaker.decision_makers))]
        if len(df_selection) == 0:  # Skip if no data for this decision maker
            continue
        ...
        for cl in classes:
            ...
            if len(df_class_selection) > 0:
                ...
                class_avgs.append(0.5)  # Default if no data for this class
...
def plot_shape_bias_boxplot(datasets,
    ...
    labels = []
    label_colors = []
    df_results_humans = None
    for dmaker in decision_maker_fun(df):
        if len(dmaker.decision_makers) == 0:  # Skip if no decision makers found
            continue
        if len(dmaker.decision_makers) > 1:
            ...
            if len(decision_maker_to_shape_bias_humans_dict) > 0:
        ...
        if len(dmaker.decision_makers) > 0 and dmaker.decision_makers[0] in consts.TORCHVISION_MODELS:
            label_colors.append(rgb(150, 150, 150))
        else:
            label_colors.append(dmaker.color)
        labels.append(dmaker.plotting_name)

    if df_results_humans is not None:
